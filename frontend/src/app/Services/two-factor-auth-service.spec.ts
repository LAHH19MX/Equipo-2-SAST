...
describe("TwoFactorAuthServiceService", () => {
  ...
  it("should verify TOTP token directly via the rest api", inject(
    [TwoFactorAuthService, HttpTestingController],
    fakeAsync(
      (service: TwoFactorAuthService, httpMock: HttpTestingController) => {
  ...
  it("should retrieve 2FA status directly via the rest api", inject(
    [TwoFactorAuthService, HttpTestingController],
    fakeAsync(
      (service: TwoFactorAuthService, httpMock: HttpTestingController) => {
        let res: any;
  ...
  it("should set up 2FA directly via the rest api", inject(
    [TwoFactorAuthService, HttpTestingController],
    fakeAsync(
      (service: TwoFactorAuthService, httpMock: HttpTestingController) => {
        let res: any;
  ...
  it("should disable 2FA directly via the rest api", inject(
    [TwoFactorAuthService, HttpTestingController],
    fakeAsync(
      (service: TwoFactorAuthService, httpMock: HttpTestingController) => {
        let res: any;
        ...
        const req = httpMock.expectOne(
          "http://localhost:3000/rest/2fa/disable",
        );
        req.flush({});
        tick();
